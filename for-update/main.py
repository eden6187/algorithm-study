...
def classify_problem(problem_categories, problem_infos):
    ...
    for problem_info in problem_infos:
        ...
        for category in categories:
            if not category in problem_categories:
                print(problem_info['dirname'] + " : 해당 디렉토리에서 문제가 발생하였습니다.", file=sys.stderr)
                print(category + ' : 해당 카테고리가 config.yml에 정의 되어 있지 않습니다.', file=sys.stderr)
                exit(1)
            classified_problem[category].append(problem_info)
...
def write_down_classified_problems_into_md(classified_problems):
    ...
    for category, infos in classified_problems.items():
        ...
        with open(file_path, 'w') as md:
            ...
            num = 1
            for info in infos:
                problem_name = info['dirname'].split('/')[-1]
                problem_path = info['dirname']
                problem_description = info['description']
                if 'blog-link' in info:
                    problem_blog_link = info['blog-link']
                else:
                    problem_blog_link = '-'
                formatted_str = index_form.format(num, problem_name, problem_path, problem_description, problem_blog_link)
                num += 1
